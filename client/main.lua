...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state['isLoggedIn'] and incinematic == false and inBathing == false and showUI then
            ...
            local temperature = Citizen.InvokeNative(0xB98B78C3768AF6E0, coords.x, coords.y, coords.z)  
            SendNUIMessage({
                action = 'hudtick',
                show = show,
                ...
                stress = stress,
                talking = talking,
                temp = math.floor(foo(temperature - 273.15) * 9/5 + 39) .. "°F", -- convert from Kelvin to Fahrenheit,
                --temp = math.floor(foo(temperature) +4) .. "°C", -- convert from Kelvin to Celsius,
                voice = voice,
